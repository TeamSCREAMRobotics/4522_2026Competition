...
public class Controlboard {
  ...
  public static Translation2d getTranslation() {
    return snapTranslationToPole(
        new Translation2d(
                -applyPower(MathUtil.applyDeadband(driveController.getLeftY(), STICK_DEADBAND), 2),
                -applyPower(MathUtil.applyDeadband(driveController.getLeftX(), STICK_DEADBAND), 2))
            .times(DrivetrainConstants.maxSpeed));
  }

  public static Translation2d getFieldRelativeTranslation() {
    return getTranslation().times(AllianceFlipUtil.getDirectionCoefficient());
  }
