...
public class SimulationThread {
  ...
  /**
   * Sets the voltage supplier used to drive the simulation each step. If {@link
   * TalonFXSubsystemSimConstants#limitVoltage()} is {@code true}, the value is clamped to ±12V.
   *
  ...
  /**
   * Manually steps the simulation. Must only be called when not using a separate thread; logs a DS
   * error and returns early if the notifier is active.
   */
  public void update() {
    if (useSeparateThread) {
