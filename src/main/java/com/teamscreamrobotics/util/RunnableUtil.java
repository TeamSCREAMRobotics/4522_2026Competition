...
public class RunnableUtil {
  ...
  /**
   * Executes a {@link Runnable} at most once (or once per triggering condition). All state is
  ...
  public static class RunOnce {
    ...
    /**
     * Runs {@code runnable} each time {@code newValue} differs from the previous call's value. Does
     * not fire on the first call (initialization).
     *
    ...
    /**
     * Fires {@code runnable} the first time {@code condition} becomes {@code true}, then again on
     * every subsequent change to {@code newValue}.
     *
  ...
  public static class RunUntil {
    ...
    /**
     * Runs {@code runnable} once if {@code stopCondition} is {@code true} and has not already
     * stopped. Marks as stopped after running.
     *
     * @param runnable the action to execute
    ...
    /**
     * Retries {@code runnable} in a loop while {@code stopCondition} is {@code true} and not yet
     * stopped, silently swallowing exceptions until it succeeds.
     *
