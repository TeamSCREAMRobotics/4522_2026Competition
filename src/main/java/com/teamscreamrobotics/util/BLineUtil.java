 ...
 *
 * <p>All methods return a {@code Commands.deferredProxy}-wrapped command so path construction is
 * deferred until schedule time, making them safe to store and re-schedule without capturing a stale
 * robot pose.
 */
public final class BLineUtil {
  ...
  public static Command driveToPose(FollowPath.Builder builder, Pose2d target) {
    return Commands.deferredProxy(() -> builder.build(new Path(new Path.Waypoint(target))))
  ...
  public static Command driveToPose(FollowPath.Builder builder, Supplier<Pose2d> target) {
    return Commands.deferredProxy(() -> builder.build(new Path(new Path.Waypoint(target.get()))))
  ...
  public static Command driveToPose(
      FollowPath.Builder builder, Pose2d target, Path.PathConstraints constraints) {
  ...
  /**
   * Drives to a dynamically-supplied pose with custom constraints; both are evaluated at schedule
   * time.
   */
  public static Command driveToPose(
      FollowPath.Builder builder, Supplier<Pose2d> target, Path.PathConstraints constraints) {
    return Commands.deferredProxy(
  ...
  public static Command driveToTranslation(FollowPath.Builder builder, Translation2d target) {
    return Commands.deferredProxy(() -> builder.build(new Path(new Path.TranslationTarget(target))))
        .withName("BLineDriveToTranslation(" + target.getX() + ", " + target.getY() + ")");
  }

  /**
   * Drives to a dynamically-supplied translation without commanding any heading change; the
   * supplier is called at schedule time.
   */
  public static Command driveToTranslation(
      FollowPath.Builder builder, Supplier<Translation2d> target) {
    return Commands.deferredProxy(
