...
/**
 * Ordered sequence of PathPlanner paths for autonomous routines. Handles alliance-aware starting
 * pose flipping and provides cursor-based path traversal.
 */
public class PPPathSequence {
   ...
   *
   * <ul>
   *   <li>{@link #RotatedField} - Use for rotationally symmetric fields (e.g. Rebuilt 2026). Calls
   *       {@link AllianceFlipUtil#FlippedPose2d()} only, which rotates 180 degrees around the field
   *       center.
   *   <li>{@link #SymmetricField} - Use for bilaterally symmetric fields where blue and red are
   *       mirrored across the length midline (y axis). Calls {@link
   *       AllianceFlipUtil#MirroredPose2d()}, transforming both alliance side and field half.
   */
   ...
   *
   * @param flipType how starting pose should be mirrored for the red alliance — see {@link
   *     FlipType}
  ...
  /**
   * Advances the internal cursor and returns a command for the next path. Starts at index 1 on
   * first call (use {@link #getStart()} for index 0). Returns a no-op {@link InstantCommand} and
   * logs a DS warning if already at the last path.
   */
  ...
  /**
   * Returns a command for the path at {@code index}. Returns a no-op {@link InstantCommand} and
   * logs a DS warning if out of bounds.
   *
  ...
  /**
   * Appends additional paths to the end of this sequence. Paths that fail to load are skipped with
   * a DS error (no exception thrown).
   *
  ...
  /**
   * Returns a new {@link PPPathSequence} with all paths mirrored across the field width centerline,
   * regardless of alliance. Use this when reusing a path designed for one half of the field on the
   * other (e.g. top wall path -> bottom wall path).
   *
