...
public class BLinePathSequence {
  ...
  /**
   * Loads and prepares a sequence of BLine paths for autonomous use. Alliance flipping is deferred
   * to retrieval time so the correct alliance is used even if the object is constructed before the
   * DS reports alliance.
   *
   ...
   */
  public BLinePathSequence(
      FollowPath.Builder builder, FieldSymmetry fieldSymmetry, String... pathNames) {
   ...
   * The path is flipped according to the current alliance at call time. Use this when you need
   * direct path access rather than a follow command.
   *
   * @param index zero-based path index
   */
  ...
  /**
   * Returns a new {@link BLinePathSequence} with all paths mirrored across the field width
   * centerline ({@code y -> fieldSizeY - y}), regardless of alliance. Use this when reusing a path
   * designed for one half of the field on the other (e.g. top wall path -> bottom wall path).
   *
