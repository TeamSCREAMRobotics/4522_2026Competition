...
/**
 * Factory for CTRE Phoenix 6 swerve drive requests with built-in heading snap and correction. Wraps
 * {@link FieldCentric}, {@link FieldCentricFacingAngle}, {@link RobotCentric}, and {@link
 * ApplyRobotSpeeds}/{@link ApplyFieldSpeeds} requests with pre-configured deadbands and drive/steer
 * request types.
 */
public class PhoenixSwerveHelper {
  ...
  /**
   * Returns a {@link FieldCentric} request with angular velocity computed from a custom PID
   * controller.
   *
   * @param translation field-relative XY velocity vector
   * @param targetAngle desired heading
  ...
  public FieldCentric getFacingAngle(
    ...
    return getFieldCentric(
        ...
        headingController.calculate(
            poseSup.get().getRotation().getRadians(), targetAngle.getRadians()));
  }
  ...
  /**
   * Returns a {@link FieldCentricFacingAngle} request using the built-in snap controller and a
  ...
  /**
   * Returns a field-centric request with automatic heading drift correction. While {@code
   * angularVelocity} is near zero the correction controller holds the last known heading; otherwise
   * the driver input is passed through and the hold angle is updated.
   *
   * @param translation field-relative XY velocity vector
