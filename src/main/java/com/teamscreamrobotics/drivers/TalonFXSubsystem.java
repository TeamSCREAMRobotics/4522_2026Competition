...
public class TalonFXSubsystem extends SubsystemBase {
  ...
  public TalonFXSubsystem(
    ...
    this.simulating =
        (Utils.isSimulation() && config.simConstants != null) || config.forceSimulation;
    master = new TalonFX(config.masterConstants.device.id);
    ...
    if (config.cancoderConstants != null) {
      cancoder = new CANcoder(config.cancoderConstants.device.id);
    ...
    for (int i = 0; i < slaves.length; ++i) {
      slaves[i] = new TalonFX(config.slaveConstants[i].device.id);
