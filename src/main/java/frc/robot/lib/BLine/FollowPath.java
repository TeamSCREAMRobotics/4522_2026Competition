 ...
 *
 * <p>This command drives the robot along a defined path by tracking translation targets
 * sequentially while simultaneously managing rotation targets. The command uses three PID
 * controllers:
 *
 * <ul>
 *   <li><b>Translation Controller:</b> Calculates command speed by minimizing total path distance
 *       remaining
 *   <li><b>Rotation Controller:</b> Controls holonomic rotation toward the current rotation target
 *   <li><b>Cross-Track Controller:</b> Minimizes deviation from the line between waypoints
 * </ul>
 *
 * <p>The path following algorithm works by:
 *
 * <ol>
 *   <li>Calculating command robot speed via a PID controller minimizing total path distance
 *       remaining
 *   <li>Determining velocity direction by pointing toward the current translation target
 *   <li>Advancing to the next translation target when within the handoff radius of the current one
 *   <li>Applying cross-track correction to stay on the line between waypoints
 *   <li>Interpolating rotation based on progress between rotation targets
 *   <li>Applying rate limiting via {@link ChassisRateLimiter} to respect constraints
 * </ol>
 *
 * <h2>Usage</h2>
 *
 ...
 *
 * <ul>
 *   <li>{@link #setPoseLoggingConsumer(Consumer)} - Log pose data
 *   <li>{@link #setDoubleLoggingConsumer(Consumer)} - Log numeric values
 *   <li>{@link #setBooleanLoggingConsumer(Consumer)} - Log boolean states
 *   <li>{@link #setTranslationListLoggingConsumer(Consumer)} - Log translation arrays
 * </ul>
 *
 * @see Path
 * @see Builder
...
public class FollowPath extends Command {
  ...
  private static Consumer<Pair<String, Translation2d[]>> translationListLoggingConsumer =
      value -> {};
   ...
   *
   * <p>The consumer receives pairs of (key, Pose2d) for various internal poses such as closest
   * points on path segments.
   *
   ...
   *
   * <p>The consumer receives pairs of (key, Translation2d[]) for data such as path waypoints and
   * robot position history.
   *
   * @param translationListLoggingConsumer The consumer to receive translation list data, or null to
   *     disable
   */
   ...
   */
  public static void setBooleanLoggingConsumer(
      Consumer<Pair<String, Boolean>> booleanLoggingConsumer) {
   ...
   *
   * <p>The consumer receives pairs of (key, Double) for various metrics such as remaining distance,
   * controller outputs, and target indices.
   *
   * @param doubleLoggingConsumer The consumer to receive double logging data, or null to disable
   */
  public static void setDoubleLoggingConsumer(
      Consumer<Pair<String, Double>> doubleLoggingConsumer) {
   ...
   *
   * <p>Passing null restores the default WPILib {@link Timer#getTimestamp()} source. This exists
   * primarily to allow deterministic unit tests without HAL timing dependencies.
   */
  ...
  public static class Builder {
     ...
     * @param robotRelativeSpeedsSupplier Supplier that returns current robot-relative chassis
     *     speeds
     * @param robotRelativeSpeedsConsumer Consumer that accepts robot-relative chassis speeds to
     *     drive
     * @param translationController PID controller for calculating command speed by minimizing path
     *     distance remaining
     ...
     *
     * <p>When the supplier returns true, the path will be flipped to the opposite alliance side
     * using {@link FlippingUtil} during command initialization.
     *
     ...
     *
     * <p>When enabled, paths will automatically be flipped when the robot is on the red alliance,
     * based on {@link edu.wpi.first.wpilibj.DriverStation#getAlliance()}.
     *
     * <p>This setting persists for future {@link #build(Path)} calls until changed.
     *
     * @return This builder for chaining
     ...
     *
     * <p>When the supplier returns true, the path will be mirrored over the vertical direction
     * across the field width ({@code y -> fieldSizeY - y}) via {@link Path#mirror()}.
     *
     * <p>This setting persists for future {@link #build(Path)} calls until changed.
     *
     * @param shouldMirrorPathSupplier Supplier returning true if the path should be mirrored
     *     vertically
     * @return This builder for chaining
     */
     ...
     *
     * <p>When set, the command will call this consumer with the path's starting pose during
     * initialization. This is useful for resetting odometry when starting autonomous routines or
     * when the robot is placed at a known location.
     *
     * <p>This setting persists for future {@link #build(Path)} calls until changed. To disable pose
     * reset on later commands when reusing the same builder, set a no-op consumer such as {@code
     * withPoseReset(pose -> {})}.
     *
     ...
     *
     * <p>When enabled, translation target handoffs occur based on projected segment progress rather
     * than raw distance, which can be more robust at higher speeds on riskier paths. Defaults to
     * false.
     *
     * <p>This setting persists for future {@link #build(Path)} calls until changed.
     *
     ...
     *
     * <p>The built command will use all the configuration from this builder. Each call to build()
     * creates an independent command that can be scheduled, using the builder's current optional
  ...
  public void initialize() {
    ...
    if (!path.isValid()) {
      logger.log(
          java.util.logging.Level.WARNING, "FollowPath: Path invalid - skipping initialization");
      return;
    }
  ...
  public void execute() {
    ...
    if (translationElementIndex >= pathElementsWithConstraints.size()
      ...
      logger.warning(
          "FollowPath: Invalid translation target after handoff at index "
    ...
    rotationElementIndex =
        rotationSelection.activeRotationIndex() >= 0
    ...
    if (rotationSelection.previousRotationIndex() >= 0
        ...
        && pathElementsWithConstraints.get(rotationSelection.previousRotationIndex()).getFirst()
            instanceof RotationTarget) {
      previousRotationElementTargetRad =
          ((RotationTarget)
    ...
    logDouble(
        "FollowPath/rotationPreviousElementIndex",
        (double) rotationSelection.previousRotationIndex());
    ...
    if (!(pathElementsWithConstraints.get(translationElementIndex).getSecond()
      ...
      logger.warning(
          "FollowPath: Expected TranslationTargetConstraint at index " + translationElementIndex);
      stopCommandedMotion();
      return;
    }
    TranslationTargetConstraint translationConstraint =
        (TranslationTargetConstraint)
            pathElementsWithConstraints.get(translationElementIndex).getSecond();

    // Clamp translation controller output as to not overpower the crossTrackController output
    // during the velo accel limiting phase
    double translationControllerOutput =
        MathUtil.clamp(
    ...
    // dont clamp cross track controller as users may prefer to tune their controller to be hyper
    // response to cross track
    ...
    if (rotationSelection.activeRotationIndex() >= 0
      ...
      RotationTarget currentRotationTarget =
          (RotationTarget)
              pathElementsWithConstraints.get(rotationSelection.activeRotationIndex()).getFirst();
      if (!(pathElementsWithConstraints.get(rotationSelection.activeRotationIndex()).getSecond()
          instanceof RotationTargetConstraint)) {
        logger.warning(
            "FollowPath: Expected RotationTargetConstraint at index " + rotationElementIndex);
        stopCommandedMotion();
        return;
      }
      rotationConstraint =
          (RotationTargetConstraint)
              pathElementsWithConstraints.get(rotationSelection.activeRotationIndex()).getSecond();
      ...
      if (currentRotationTarget.profiledRotation()) {
        ...
        Translation2d rotationStart =
            rotationSelection.previousRotationIndex() < 0
                ? pathInitStartPose.getTranslation()
                : calculateRotationTargetTranslation(rotationSelection.previousRotationIndex());
        Translation2d rotationEnd =
            calculateRotationTargetTranslation(rotationSelection.activeRotationIndex());
        ...
        double segmentProgress =
            rotationSegmentLength < SEGMENT_EPSILON
                ? 1.0
                : calculateSegmentProjectionT(
        ...
        double rotationDifference =
            MathUtil.angleModulus(endRotation - previousRotationElementTargetRad);
    ...
    if (logCounter++ % 3 == 0) {
      ...
      translationListLoggingConsumer.accept(
          new Pair<>(
              "FollowPath/robotTranslations", robotTranslations.toArray(Translation2d[]::new)));
    }
    ...
    logDouble(
        ...
        Math.toDegrees(currentRotationTargetRad.minus(currentPose.getRotation()).getRadians()));
   ...
   *
   * <p>This intentionally supports "draining" through multiple targets in one cycle, which avoids
   * one-cycle stalls on chains of tiny/degenerate segments.
   */
  private void advanceTranslationTargets(Pose2d currentPose) {
    while (true) {
      ...
      TranslationTarget currentTranslationTarget =
          (TranslationTarget) pathElementsWithConstraints.get(translationElementIndex).getFirst();
      ...
      TranslationSegmentState currentSegment = getCurrentTranslationSegmentState(currentPose);
      if (!shouldHandoffTranslationTarget(
          currentPose, currentTranslationTarget, currentSegment, handoffRadius)) {
        return;
      }
  ...
  private boolean shouldHandoffTranslationTarget(
    ...
    double distanceToTarget =
        currentPose.getTranslation().getDistance(currentTranslationTarget.translation());
   ...
   *
   * <p>If the cursor is invalid, returns a degenerate segment at the robot position so callers can
   * handle the error path uniformly.
   */
  private TranslationSegmentState getCurrentTranslationSegmentState(Pose2d currentPose) {
    ...
    double segmentProgress =
        segmentLength < SEGMENT_EPSILON
            ? 1.0
            : calculateSegmentProjectionT(
                startTranslation, endTranslation, currentPose.getTranslation());
   ...
   *
   * <p>Selection rule: 1) If current segment has candidates, choose by segment progress. 2) If
   * current segment is degenerate, choose highest t_ratio candidate. 3) If no current-segment
   * candidate exists, look ahead to the next segment with a target.
   */
  ...
  private double getMaxTRatioOnSegment(int segmentEndTranslationIndex) {
    ...
    for (int i = 0; i < pathElementsWithConstraints.size(); i++) {
      ...
      double clampedTRatio =
          clampTRatio(((RotationTarget) pathElementsWithConstraints.get(i).getFirst()).t_ratio());
  ...
  private Translation2d getTranslationAtIndex(int translationIndex) {
    if (translationIndex >= 0
        ...
        && pathElementsWithConstraints.get(translationIndex).getFirst()
            instanceof TranslationTarget) {
      return ((TranslationTarget) pathElementsWithConstraints.get(translationIndex).getFirst())
          .translation();
    }
   ...
   *
   * <p>This is used by the translation controller to calculate command speed. Sums the distances
   * from the current position through all remaining translation targets.
   *
  ...
  private double calculateRemainingPathDistance() {
    ...
    for (int i = translationElementIndex; i < pathElementsWithConstraints.size(); i++) {
      if (pathElementsWithConstraints.get(i).getFirst() instanceof TranslationTarget) {
        remainingDistance +=
            previousTranslation.getDistance(
                ((TranslationTarget) pathElementsWithConstraints.get(i).getFirst()).translation());
        previousTranslation =
            ((TranslationTarget) pathElementsWithConstraints.get(i).getFirst()).translation();
      }
   ...
   *
   * <p>Positive values indicate the robot is to the right of the path, negative values indicate the
   * robot is to the left of the path.
   *
   ...
   *
   * <p>This walks backward from the current translation element to find the previous translation
   * target. If none exists, it falls back to the path initialization pose. This keeps cross-track
   * calculations stable when translation targets switch.
   *
  ...
  private Translation2d calculateRotationTargetTranslation(int index) {
    ...
    if (bounds == null) {
      logger.warning(
          "FollowPath: Missing translation bounds for rotation target at index " + index);
      return new Translation2d();
    }
    ...
    Translation2d pointOnSegment =
        new Translation2d(
            bounds.startTranslation().getX()
                + (bounds.endTranslation().getX() - bounds.startTranslation().getX()) * tRatio,
            bounds.startTranslation().getY()
                + (bounds.endTranslation().getY() - bounds.startTranslation().getY()) * tRatio);
  ...
  private RotationSegmentBounds getRotationSegmentBounds(int rotationIndex) {
    ...
    return new RotationSegmentBounds(
        startTranslationIndex, endTranslationIndex, startTranslation, endTranslation);
  }
  ...
  private boolean isEventTriggerTRatioReached(int eventIndex, Pose2d currentPose) {
    ...
    if (translationA == null || translationB == null) {
      logger.warning(
          "FollowPath: Missing translation bounds for event trigger at index " + eventIndex);
      return false;
    }
    ...
    double segmentProgress =
        calculateSegmentProjectionT(translationA, translationB, currentPose.getTranslation());
  ...
  public boolean isFinished() { // TODO add final velocity tolerance
    ...
    boolean rotationAtSetpoint =
        Math.abs(currentRotationTargetRad.minus(poseSupplier.get().getRotation()).getRadians())
            < Math.toRadians(path.getEndRotationToleranceDeg());
    boolean finished =
        isLastRotationElement
   ...
   *
   * <p>This index represents the current rotation target being tracked, where both waypoint
   * rotations and standalone rotation targets are counted together.
   *
   * @return The current rotation element index (0-based), or -1 when no active rotation target
   *     remains
   */
   ...
   *
   * <p>This index represents the current translation target being tracked, where both waypoint
   * translations and standalone translation targets are counted together.
   *
   ...
   *
   * <p>This value is computed live from the command's current traversal cursor and translation
   * targets. It mirrors the distance basis used by the translation controller during execution.
   *
   * <p>Returns {@code 0.0} when the command is not in a valid traversal state (for example, invalid
   * path or uninitialized/invalid translation cursor).
   *
