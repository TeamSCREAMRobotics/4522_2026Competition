 ...
 *
 * <p>This class provides acceleration limiting for both translational and rotational velocities in
 * field-relative coordinates. It ensures that the robot's movement remains within physical
 * constraints and prevents sudden changes in velocity that could cause wheel slip or mechanical
 * stress.
 *
 * <p>The limiter operates in two phases:
 *
 * <ol>
 *   <li>Velocity limiting: Clamps the desired speeds to maximum allowed velocities
 *   <li>Acceleration limiting: Limits the rate of change from the previous velocity state
 * </ol>
 *
 * <p>Example usage:
 *
...
public class ChassisRateLimiter {
   ...
   *
   * @param desiredSpeeds The target chassis speeds to achieve. This object may be modified if
   *     velocity limiting is applied.
   ...
   * @param maxTranslationalVelocityMetersPerSec The maximum allowed translational velocity in
   *     meters per second. Set to 0 or negative to disable velocity limiting.
   * @param maxAngularVelocityRadiansPerSec The maximum allowed angular velocity in radians per
   *     second. Set to 0 or negative to disable velocity limiting.
   * @return A new {@link ChassisSpeeds} object with velocities constrained to the specified limits.
   *     The returned speeds will not exceed the maximum velocities and the change from {@code
   *     lastSpeeds} will not exceed the maximum accelerations.
   */
  public static ChassisSpeeds limit(
    ...
    if (maxTranslationalVelocityMetersPerSec > 0 && maxAngularVelocityRadiansPerSec > 0) {
      double desiredVelocity = Math.hypot(desiredSpeeds.vx, desiredSpeeds.vy);
    ...
    double desiredAcceleration =
        Math.hypot(desiredSpeeds.vx - lastSpeeds.vx, desiredSpeeds.vy - lastSpeeds.vy) / dt;

    double obtainableAcceleration =
        MathUtil.clamp(desiredAcceleration, 0, maxTranslationalAccelerationMetersPerSec2);

    double theta = Math.atan2(desiredSpeeds.vy - lastSpeeds.vy, desiredSpeeds.vx - lastSpeeds.vx);

    double desiredOmegaAcceleration = (desiredSpeeds.omega - lastSpeeds.omega) / dt;

    double obtainableOmegaAcceleration =
        MathUtil.clamp(
